...
Template.boardList.events({
  ...
  'dragover .js-board'(evt) {
    // Skip if a workspace node is being dragged (check types, not getData, which is blocked during dragover)
    const types = evt.originalEvent.dataTransfer.types;
    if (types && (types.includes('application/x-workspace-id') || Array.from(types).includes('application/x-workspace-id'))) return;
    // Skip multi-board drag (no sort for multi-selection)
    if (types && (types.includes('application/x-board-multi') || Array.from(types).includes('application/x-board-multi'))) return;
    ...
    if (evt.clientY < rect.top + rect.height / 2) {
      targetEl.classList.add('board-drag-over-top');
      targetEl.classList.remove('board-drag-over-bottom');
    } else {
      targetEl.classList.add('board-drag-over-bottom');
      targetEl.classList.remove('board-drag-over-top');
    }
  },
  'dragleave .js-board'(evt) {
    evt.currentTarget.classList.remove('board-drag-over-top', 'board-drag-over-bottom');
  },
  'dragend .js-board'() {
    document.querySelectorAll('.js-board').forEach((el) => {
      el.classList.remove('board-drag-over-top', 'board-drag-over-bottom');
    });
  },
  'drop .js-board'(evt) {
    evt.preventDefault();
    ...
    const targetEl = evt.currentTarget;
    targetEl.classList.remove('board-drag-over-top', 'board-drag-over-bottom');

    // Ignore workspace drags
    const workspaceId = evt.originalEvent.dataTransfer.getData('application/x-workspace-id');
    if (workspaceId) return;
    // Ignore multi-board drag (sorting not supported for multi-selection)
    const isMultiBoard = evt.originalEvent.dataTransfer.getData('application/x-board-multi');
    if (isMultiBoard) return;

    const draggedBoardId = evt.originalEvent.dataTransfer.getData('text/plain');
    if (!draggedBoardId) return;

    const targetBoard = Blaze.getData(targetEl);
    if (!targetBoard || draggedBoardId === targetBoard._id) return;

    // Determine whether to place before or after the target board
    const rect = targetEl.getBoundingClientRect();
    const dropBefore = evt.clientY < rect.top + rect.height / 2;

    let prevBoardDom = null;
    let nextBoardDom = null;

    if (dropBefore) {
      nextBoardDom = targetEl;
      let prev = targetEl.previousElementSibling;
      while (prev && !prev.classList.contains('js-board')) {
        prev = prev.previousElementSibling;
      }
      prevBoardDom = prev || null;
    } else {
      prevBoardDom = targetEl;
      let next = targetEl.nextElementSibling;
      while (next && !next.classList.contains('js-board')) {
        next = next.nextElementSibling;
      }
      nextBoardDom = next || null;
    }

    const sortIndex = Utils.calculateIndex(prevBoardDom, nextBoardDom, 1);
    const currentUser = ReactiveCache.getCurrentUser();
    if (currentUser && typeof currentUser.setBoardSortIndex === 'function') {
      currentUser.setBoardSortIndex(draggedBoardId, sortIndex.base);
    }
  },
