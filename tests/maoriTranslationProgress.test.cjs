...
assert.match(maori['api-report-desc'], /REST API/);
assert.match(maori['api-no-calls'], /WITH_API=true/);
assert.match(maori['recovery-report-desc'], /MongoDB/);
assert.equal(maori['carbon-copy'], 'Tārua Waro (Cc:)');
assert.equal(maori['help-request'], 'Tono Āwhina');
assert.match(maori['Node_heap_malloced_memory'], /Node.*malloc/);
assert.match(maori['Node_memory_usage_rss'], /Node/);
assert.match(maori['custom-legal-notice-link-url'], /URL/);
assert.equal(maori['copyChecklist'], 'Tārua Rārangi Arowhai');
assert.match(maori['attachment-move-storage-gridfs'], /GridFS/);
assert.match(maori['attachment-move-storage-s3'], /S3/);
assert.match(maori['attachment-repair-locations-description'],
  /GridFS.*kapua/);
assert.equal(maori['avatars'], 'Ngā Whakaahua Kaiwhakamahi');
assert.match(maori['gridfs-file-id'], /ID.*GridFS/);
assert.match(maori['mongodb-compact-description'], /MongoDB GridFS.*Compact/);
assert.match(maori['mongodb-compact-warning'], /Compact.*oplog.*Meteor/s);
assert.match(maori['Mongo_sessions_count'], /Mongo/);
assert.match(maori['preview-pdf-not-supported'], /PDF/);
assert.deepEqual(tokens(maori['drag-board-to-workspace']), ['__workspaces__']);
...
assert.match(maori['migrate-all-to-gridfs'], /GridFS/);
assert.match(maori['migrate-all-to-s3'], /S3/);
assert.match(maori['migration-batch-size-description'], /1-100/);
