...
assert.equal(greenlandic['cards-loading-all'], 'Kortsit tamarmik');
assert.equal(greenlandic['anonymized-user'], 'atuisoq');
assert.equal(greenlandic.backup, 'Backupi');
assert.equal(greenlandic['backup-frequency-daily'], 'Ullut tamaasa');
...
assert.equal(greenlandic['lost-cards'], 'Kortsit tammartut');
assert.equal(greenlandic['migration-progress-status'], 'Killiffik');
assert.equal(greenlandic['step-validate-migration'],
  'Nuussineq uppernarsaruk');
assert.equal(greenlandic['step-fix-attachment-urls'],
  'Ilanngussat URL-ii aaqqikkit');
assert.equal(greenlandic['job-queue'], 'Suliassat utaqqisut');
assert.equal(greenlandic['filesystem-storage'],
  'Filsystemimi toqqorsivik');
assert.equal(greenlandic['memory-usage'], 'Eqqaissutissap atuinera');
assert.equal(greenlandic['migration-steps'], 'Nuussinerup alloriarneri');
assert.equal(greenlandic['system-resources'], 'Systemip isumalluutai');
assert.equal(greenlandic.server, 'Serveri');
assert.deepEqual(tokens(greenlandic['repair-broken-cards-done']),
  ['__fixed__']);
