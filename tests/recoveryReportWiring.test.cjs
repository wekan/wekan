...
test('client report is wired: config, columns, menu, rendering', () => {
  ...
  assert.ok(/'change \.js-table-page-filter'[\s\S]*cfg\.filter\.set\(\$\(event\.currentTarget\)\.val\(\) \|\| 'all'\)[\s\S]*loadReport\(reportId, \{ recount: true \}\)/.test(js),
    'Recovery filter reloads through the shared report dropdown handler');
