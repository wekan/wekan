...
test('both Greek tags translate every report placeholder', () => {
  for (const code of greek) {
...
test('all translated report values contain Greek script', () => {
  for (const code of greek) {
    const lang = read(code);
    for (const key of reportKeys) assert.match(lang[key], /[Ͱ-Ͽ]/, code + ' lacks Greek in ' + key);
  }
});
...
console.log('\nupcomingGreekTranslations: ' + passed + ' tests passed');
