...
assert.match(chuvash['list-width-error-message'], /пӗтӗм хисеп ≥ 200 пиксель/);
assert.doesNotMatch(chuvash['list-width-error-message'], /270/);
...
assert.match(chuvash['api-report-desc'], /REST API/);
...
assert.match(chuvash['attachment-repair-locations-description'], /GridFS/);
...
assert.match(chuvash['backup-time'], /HH:MM/);
assert.match(chuvash['gcs-permissions-note'], /client_email.*Storage Object Admin/);
assert.match(chuvash['s3-endpoint-menu-path'], /Cloudflare R2.*Backblaze B2.*Wasabi/);
assert.match(chuvash['s3-secret-key-menu-path'], /\.csv/);
assert.match(chuvash['gcs-credentials-menu-path'], /IAM & Admin.*JSON/);
assert.match(chuvash['gridfs-enabled-description'], /MongoDB GridFS/);
assert.match(chuvash['gridfs-move-collectionfs-note'], /CollectionFS/);
assert.match(chuvash['s3-region-description'], /us-east-1/);
assert.match(chuvash['s3-ssl-enabled-description'], /SSL\/TLS/);
assert.match(chuvash['restore-lost-cards-migration-description'], /swimlaneId.*listId/);
