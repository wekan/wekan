...
assert.match(buryat['backup-time'], /HH:MM/);
assert.match(buryat['gcs-permissions-note'], /client_email.*Storage Object Admin/);
assert.match(buryat['gcs-key-filename-menu-path'], /JSON/);
assert.match(buryat['gridfs-enabled-description'], /MongoDB GridFS/);
assert.match(buryat['s3-region-description'], /us-east-1/);
assert.match(buryat['s3-ssl-enabled-description'], /SSL\/TLS/);
assert.match(buryat['restore-lost-cards-migration-description'], /swimlaneId.*listId/);
assert.match(buryat['fix-all-file-urls-migration-description'], /URL/);
assert.match(buryat['run-restore-all-archived-migration-confirm'], /ID/);
assert.match(buryat['step-fix-missing-ids'], /ID/);
assert.match(buryat['step-fix-file-urls'], /URL/);
assert.equal(buryat['every-30-minutes'], '30 минута бүри');
