...
(async () => {
  ...
  for (const review of reviews.filter(row => row.key === 'r-items-list' && !['sk', 'gl', 'gl-ES', 'an', 'eo', 'br', 'oc', 'vo', 'wa', 'wo', 'zgh'].includes(row.locale))) {
  ...
  assert.equal(reviews.find(row => row.locale === 'br' && row.key === 'r-items-list').value, 'elfenn1,elfenn2,elfenn3', 'Breton example item names remain translated; commas preserve input syntax');
