...
assert.match(data['list-width-error-message'], /amplaria d’a lista ha d’estar un entero ≥ 200 pixels\.$/);
assert.doesNotMatch(data['list-width-error-message'], /270|mayor que/u);
...
assert.equal(data['team-name-not-found'], "Equipo '%s' no trobau.");
assert.equal(data['theme-default'], 'Por defecto (sin sobreescribir)');
assert.equal(data['translation-number'], 'O numero de cadenas de traducción personalizadas ye:');
