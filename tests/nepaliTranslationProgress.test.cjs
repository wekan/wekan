...
assert.match(nepali['api-report-desc'], /REST API/);
assert.match(nepali['api-no-calls'], /REST API.*WITH_API=true.*API/);
...
assert.match(nepali['restore-lost-cards-migration-description'],
  /swimlaneId.*listId/);
assert.match(nepali['restore-all-archived-migration-description'],
  /swimlaneId.*listId/);
