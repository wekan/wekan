...
test('amd64 and arm64 are REQUIRED; the exotic arches are OPTIONAL; loong64 is neither', () => {
  ...
  for (const need of ['linux/ppc64le', 'linux/s390x', 'linux/riscv64', 'linux/386', 'linux/arm/v7']) {
...
test('the Dockerfile can install Node for every candidate platform, with the right bundle', () => {
  ...
  // And the bundle arch the Decide step pairs with each platform must be the one
  // the Dockerfile's case maps that TARGETARCH to (WEKAN_ARCH), or the image would
  // download another CPU's .zip. e.g. linux/386 -> TARGETARCH 386 -> WEKAN_ARCH i386.
  const wekanArch = { '386': 'i386', arm: 'armhf' };  // the ones that differ
  for (const { plat, arch } of [...req, ...opt]) {
    const ta = targetArch[plat];
    assert.ok(ta, `test needs a TARGETARCH mapping for ${plat}`);
    assert.ok(new RegExp(`"${ta}"\\)`).test(dockerfile),
      `Dockerfile arch case has no "${ta}") branch for ${plat}`);
    const expectBundle = wekanArch[ta] || ta;
    assert.strictEqual(arch, expectBundle,
      `Decide step pairs ${plat} with bundle "${arch}", but its TARGETARCH ${ta} maps to WEKAN_ARCH "${expectBundle}"`);
  }
});
