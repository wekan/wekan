...
for (const [key, value] of Object.entries(bashkir)) {
  if (value !== english[key]) assert.deepEqual(tokens(value), tokens(english[key]), key);
  assert.deepEqual(tags(value), tags(english[key]), key);
}
assert.equal(bashkir.accept, 'Ҡабул итергә');
assert.match(bashkir['act-addChecklistItem'], /[ӘәӨөҮүҠҡҒғҘҙҪҫҺһ]/);
...
assert.match(bashkir['advanced-filter-description'], /== != <= >= && \|\| \( \).*F1 == \/Tes\.\*\/i/);
assert.match(bashkir['import-board-instruction-issues'], /__sourceName__.*__endpoint__/);
