...
assert.deepEqual(tokens(konkani['repair-broken-cards-done']), ['__fixed__']);
assert.deepEqual(tokens(konkani['repair-broken-cards-done-unfixable']),
  ['__fixed__', '__unfixable__']);
assert.deepEqual(tokens(konkani['globalSearch-instructions-operator-number']),
  ['__operator_number__']);
...
assert.deepEqual(tokens(konkani['import-dependencies-done']),
  ['__imported__', '__unmatched__']);
assert.deepEqual(tokens(konkani['background-too-big']), ['{{size}}']);
assert.deepEqual(tokens(konkani['custom-field-stringtemplate-format']),
  ['%{value}']);
...
assert.equal(konkani.copied, 'नकल केलें!');
assert.equal(konkani['move-storage-fs'], 'फायल प्रणाली');
assert.equal(konkani['mongodb-compact-run'], 'MongoDB Compact चलयात');
assert.deepEqual(tokens(konkani['drag-board-to-workspace']), ['__workspaces__']);
assert.equal(konkani.progress, 'प्रगती');
...
assert.deepEqual(tags(konkani['render-links-as-plain-text-description']),
  ['<a href>']);
assert.equal(konkani['disable-all-export'], 'सगळी निर्यात बंद करात');
assert.equal(konkani.backup, 'बॅकअप');
assert.equal(konkani['backup-now'], 'आतां बॅकअप करात');
...
assert.equal(konkani.steps, 'पांवडे');
assert.equal(konkani['migration-progress-overall'], 'एकूण प्रगती');
assert.equal(konkani['overall-progress'], 'एकूण प्रगती');
assert.equal(konkani['migration-steps'], 'स्थलांतर पांवडे');
assert.equal(konkani['current-step'], 'सद्य पांवडो');
assert.equal(konkani['account-created'],
  'खातें तयार जालें! आतां प्रवेश करुंक मेळटा.');
