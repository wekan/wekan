...
assert.match(malayalam['accounts-lockout-settings'], /ബ്രൂട്ട് ഫോഴ്‌സ്/);
assert.match(malayalam['accounts-lockout-period'], /സെക്കൻഡുകൾ/);
assert.equal(malayalam['admin-people-filter-active'], 'സജീവം');
assert.match(malayalam['attachment-storage-configuration'], /അറ്റാച്ച്മെന്റ്/);
assert.match(malayalam['board-backup-scheduled'], /ബാക്കപ്പ്/);
assert.deepEqual(tokens(malayalam['database-migration-confirm']), ['__db__']);
for (const literal of ['mongodb://127.0.0.1:27018',
  'mongodb://127.0.0.1:27019', 'WEKAN_FERRETDB_URL', 'WEKAN_MONGODB_URL',
  'MONGO_URL']) {
  assert.match(malayalam['database-migration-description'],
...
assert.match(malayalam['backup-time'], /HH:MM/);
assert.match(malayalam['backup-day-of-month'], /1-28/);
assert.match(malayalam['gcs-permissions-note'], /client_email/);
...
for (const key of ['restore-lost-cards-migration-description',
  ...
  assert.match(malayalam[key], /swimlaneId/);
...
assert.deepEqual(tokens(malayalam['operator-number-expected']),
  ['__operator__', '__value__']);
assert.deepEqual(tokens(malayalam['globalSearch-instructions-operator-has']),
  tokens(english['globalSearch-instructions-operator-has']));
assert.deepEqual(tokens(malayalam['globalSearch-instructions-notes-3-2']),
...
assert.match(malayalam['api-report-desc'], /REST API/);
assert.match(malayalam['api-no-calls'], /WITH_API=true/);
assert.match(malayalam['recovery-report-desc'], /MongoDB/);
