...
assert.match(kyrgyz['migrate-all-to-gridfs'], /GridFS/);
assert.match(kyrgyz['migrate-all-to-s3'], /S3/);
assert.match(kyrgyz['migration-batch-size-description'], /1-100/);
assert.match(kyrgyz['migration-cpu-threshold'], /CPU.*%/);
assert.match(kyrgyz['migration-cpu-threshold-description'], /CPU.*10-90/);
assert.match(kyrgyz['migration-delay-ms-description'], /100-10000/);
assert.match(kyrgyz['migration-info-text'],
  /бир жолу.*иштөө ылдамдыгын.*фондо/);
