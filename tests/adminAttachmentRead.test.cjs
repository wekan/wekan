...
function policy(user) {
  return vm.runInNewContext(`${resolver}\n${guard}\nisAuthorizedForBoard`, {
    require: () => ({ activeUserByToken: async () => user?.loginDisabled ? null : user }),
...
test('ordinary private-board access still requires membership', async () => {
  for (const [user, expected] of [[{ _id: 'member' }, true], [{ _id: 'member', loginDisabled: true }, false], [{ _id: 'outsider' }, false], [null, false]]) {
