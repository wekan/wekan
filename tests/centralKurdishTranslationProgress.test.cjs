...
assert.match(kurdish['list-width-error-message'], /ژمارەیەکی تەواوی لانی کەم 200 پیکسڵ/);
assert.doesNotMatch(kurdish['list-width-error-message'], /270/);
...
assert.match(kurdish['accounts-lockout-known-users'], /وشەی نهێنی/);
...
assert.match(kurdish['backup-time'], /HH:MM/);
assert.match(kurdish['backup-day-of-month'], /1-28/);
...
assert.match(kurdish['restore-lost-cards-migration-description'], /swimlaneId.*listId/);
