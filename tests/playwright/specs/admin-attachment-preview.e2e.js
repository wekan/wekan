...
for (const extension of ['docx', 'png', 'pdf']) {
test(`Files Report previews a private ${extension} file for a non-member administrator only`, async ({ page, request, adminUser, user2, board }) => {
  const previewErrors = [];
  ...
  try {
    ...
    const type = { docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', png: 'image/png', pdf: 'application/pdf' }[extension];
