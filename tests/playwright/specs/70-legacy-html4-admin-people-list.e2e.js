...
test('People list, filters, state and locations match at the same URL', async ({ browser, baseURL }) => {
  ...
  try {
    ...
    let form = legacy.locator(`form:has(input[name="legacyOperation"][value="show-login-country"]):has(input[name="targetUserId"][value="${target.id}"])`);
    ...
    db.cleanup({ userIds: [target.id, ...(admin?._id ? [admin._id] : [])] });
  }
});
