...
assert.equal(inuktitut['sandstorm-storage-item'], 'ᑐᖅᑯᖅᓯᕕᒃ');
assert.match(inuktitut['cards-loading-description'], /CARDS_LOADING_LAZY_THRESHOLD/);
assert.deepEqual(tags(inuktitut['render-links-as-plain-text-description']),
  ['<a href>']);
...
assert.equal(inuktitut['unmigrated-boards'],
  'ᓅᑕᐅᓯᒪᙱᑦᑐᑦ ᐊᓪᓚᕖᑦ');
assert.equal(inuktitut['otp-required'], 'OTP ᓇᓗᓇᐃᒃᑯᑕᖅ ᐱᔭᕆᐊᓕᒃ');
...
assert.deepEqual(tokens(inuktitut['custom-field-stringtemplate-format']),
  ['%{value}']);
...
assert.equal(inuktitut['accounts-lockout-locked-users'],
  'ᐊᑐᖅᑏᑦ ᐸᕝᕕᓴᖅᑕᐅᔪᑦ');
