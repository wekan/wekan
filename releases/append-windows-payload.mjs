...
export function manifestHeader(members) {
  if (members.length === 0) {
    throw new Error(
      'the archive has no executable, native addon, or startup member to unpack');
  }
...
export function archiveMemberNames(listing) {
  // bsdtar on the Windows runner prints ZIP member separators as backslashes.
  // Archive member names are portable POSIX paths everywhere else in this
  // module, so normalize the listing at its boundary.
