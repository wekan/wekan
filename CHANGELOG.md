...
This release expands the REST API (Admin API, board member, card field, card
copy/move). Work is in progress; the items below document current state,
planned additions, and remaining design questions. REST routes live in
`server/models/*.js` (using `WebApp.handlers.get/post/put/delete`), their schemas
in `models/*.js`, OpenAPI docs are generated by `openapi/generate_openapi.py`
...
- **Add/Remove board member to card as card member or card assignee.**
  ...
  `addcardassignee`/`removecardassignee`. Thanks to xet7 and Claude.
...
- **Fix Issue [#5846](https://github.com/wekan/wekan/issues/5846): Add/Remove card
  ...
  or pass an empty string, to clear). Thanks to xet7 and Claude.
...
- **Issue [#3062](https://github.com/wekan/wekan/issues/3062): API for the Card
  ...
  `setcardsetting`. Thanks to xet7 and Claude.
...
- **Fix Issue [#4743](https://github.com/wekan/wekan/issues/4743): Using the REST API
  ...
  storms persist. Thanks to xet7 and Claude.
...
- **`rebuild-wekan.sh` test menu reorganized.** There is now a menu option per test
  ...
  --driver-package meteortesting:mocha`). Thanks to xet7 and Claude.
