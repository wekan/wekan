...
| Account preferences and information | `/account/profile` is complete: the Jade popup and semantic HTML4 form share one atomic guarded service for Full Name, Username, Initials and Email. Admin policy controls identity-field editing and is repeated server-side; uniqueness, email verification state, OAuth2 refusal and Security reporting are enforced. `/account/language` is complete: both views share all native language names/RTL metadata and one exact-tag service; the saved choice controls the immediate HTML4 response, later cookieless pages and the direct Jade route, while injected unknown tags are Security-reported. `/account/password` is complete: both forms share naturally ordered current/new/repeated-password fields and one bounded bcrypt-verifying service; mismatch and wrong-current negative paths cannot alter the secret, reusable Meteor tokens are revoked and repeated failures are account/address throttled. `/account/settings` is complete: its six desktop/card preferences use one fixed-field, typed, range-checked service shared by Jade's individual controls and the grouped HTML4 Save form; worker-hidden and injected values are refused and reported. `/account/color` and `/account/font` are complete: both expose the same shared theme/font/size catalogs, custom colors and All Boards preference through one strict account-bound service; unavailable HTML4 fonts fall back normally, fixed themes ignore non-applicable color fields, malformed or injected CSS-facing values are refused and reported, and defaults remain explicitly selectable. `/account/avatar` is complete: HTML4 uses a bounded multipart upload and private owner-checked embedded GIF thumbnails, while both views share ID-scoped selection and confirmed deletion; upload blocking, size/decode failure, foreign IDs and selected-file pointer cleanup are enforced at the common boundary. Same-URL tests prove persisted positive behavior, paired views and forged-policy/tag/wrong-secret/setting/theme/font/avatar refusals. Invitation and logout routes remain |
