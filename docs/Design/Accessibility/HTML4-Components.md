...
| Destructive confirmation | exact object scope, permission, warning and audit context | two independently signed POST controls plus Cancel | popup confirmation calling the same acknowledged operation |
...
| Persisted background job | owner, state, progress, bounded results/errors, permitted transitions | textual state/count plus signed Resume, Cancel, confirmed Delete and Clear controls | reactive progress bar, result list and matching controls |
...
| --- | --- |
| `/sign-in`, `/sign-up`, `/forgot-password`, `/reset-password/:token`, `/enroll-account/:token`, `/verify-email/:token`, `/send-again` | Complete public account lifecycle: cookieless sign-in and registration; uniform password-recovery and verification-message requests; and bounded, atomic, single-use reset, enrollment and email-verification tokens. Naturally ordered labelled forms work without JavaScript or cookies, token refusals are independently throttled and Security-reported, two-factor accounts return to Sign In, and paired same-URL browser tests prove persisted success plus replay refusal |
...
| `/b/...` | Dedicated upper-left board and semantic card-detail readers including comments, checklists, subtasks, activities and attachments. Writable boards share authorized services with Meteor and provide signed create, step/top/bottom ordering, full cross-board move/copy placement, list movement, exact numeric sort order, read-only setting-gated Card Number, title, description, color, labels, Members, Assignees, Requested By, Assigned By, locations, stickers, dependencies, voting, Planning Poker, due completion, spent/overtime, Watch, parent-card assignment, all seven Custom Field types, four ISO 8601 dates, the member Date Format preference, archive and restore controls. Card destination forms use native title/destination/position fields and the same exact-scope service as Jade; forged boards, lists, swimlanes and relative cards are rejected before writes and Security-reported. Card Number is rendered with the real content title in both views and is never caller-supplied. Date Format uses the same three translated choices and strict account-bound service in both views, with forged values Security-reported. Dependencies expose textual links and native add/edit/remove forms for the shared relation, icon and color catalogs. Voting exposes question/audience/privacy/deadline management, public names or private counts, desired-state participation and confirmed removal. Planning Poker exposes its audience/deadline, ten ASCII choices, private in-progress state, closed counts and names, administrator finish/replay/estimation and confirmed removal controls. Completion, time, Watch and parent assignment use desired-state or labelled native controls and linked-content-aware writes. The current parent is a readable cross-board link; a shared visibility and persisted-ancestor check rejects forged private parents and cycles before writing. Subtasks expose readable cross-board links plus native create, title, up/down ordering and confirmed archive controls. Their shared service binds every child to the route's real linked-content parent, validates active destination placement and requires board administration for archive. The newest 50 card activities use one plain-text translation descriptor in HTML4 and as each Jade activity's accessible name. They remain board-admin-only, obey the global hide setting, and repeat linked-content board visibility before querying. Card History shares Jade's searchable, contributor-filtered, paged history service and content presentation; semantic row checkboxes and signed Restore controls retain integrity verification, while a URL-bound exact-card guard refuses and Security-reports cross-card rows. Jade people/identity/label/color/date/sort/location/sticker/dependency/vote/poker/completion/time/Watch/parent/subtask/Custom Field controls share strict operations; empty values clear text/color/dates/time/parent. Worker self-assignment remains available without granting general writes. Comment-capable members can add comments and replies; parent text is explicit, while authors and permitted admins can edit or confirm-delete them. Reactions use the shared safe catalog and authorized toggle service, with ASCII state, count and member names. Card and checklist PDF/Excel/JSON/ZIP export and JSON/ZIP import use the same part catalog, sanitizer, default attachment storage and exact-scope services as HTML5; card downloads have replay-safe purpose signatures that do not invalidate adjacent forms. Core checklist creation, rename, ordering, settings, item state, copy/move destination, item-to-card and confirm-delete operations use the same authorized services as HTML5; HTML4 renders progress and state in text. Image attachments have server-stored GIF previews and every attachment has an original download through distinct purpose-signed POST controls. PDF and office documents share the bounded server converter, selectable plain text/safe Excel tables, stored GIF images and native paged controls with HTML5. Text and JSON share a 2 MiB bounded, escaped selectable-text preview. Audio/video have purpose-bound inline responses for native or external players. Attachment rename, image cover toggle and confirmed delete use the same exact-scope service as HTML5 and report forged scopes. Remaining card fields remain |
...
| `/import`, `/import/:source` | Complete. Shared source picker and import-part toggles; bounded signed JSON/CSV text and streamed JSON, Excel, Trello ZIP and WeKan ZIP files call their common sanitized importers. Whole-board JSON/CSV member mapping uses a short-lived session-bound sanitized draft, source-ordered labelled native username fields, exact-username suggestions, validated target accounts and an atomic single-use finish; Excel and multi-board ZIP retain Jade's direct no-mapping path, and WeKan ZIP attachments stream into Default Storage. Trello JSON and multi-board ZIP imports preserve the same optional named personal-workspace destination as Jade through a bounded server-only draft and authenticated find/create/assign operation. Trello API credentials remain write-only; saved credentials drive server-fetched workspace/board discovery, native board selection, validated local parent-workspace selection and persisted owner-scoped jobs. HTML4 renders progress/results/errors and Resume, Cancel, confirmed Delete Imported and Clear operations through the same Meteor methods as Jade; forged IDs and protected transitions are Security-reported |
...
| Admin Panel pages and panes | All 20 `/admin/problems/*` panes have dedicated Global Admin-only controllers sharing the modern services, authorization and reporting boundaries. Summary retains status, acknowledgement and repair operations; Performance retains its Card-loading guidance; Security, Delete and Notifications retain their audited settings; and every report retains its modern columns, bounded search/paging, filters and relationship resolution. Files also retains purpose-bound preview/original controls and setting-gated audited permanent delete. A registry-derived source audit requires exactly 20/20 mappings, so no Problems pane can regress to the generic fallback. All seven Settings panes now have dedicated controllers and same-URL no-JavaScript/modern browser tests with screenshot pairs. Version shares statistics and manifest checks; Announcement and Accessibility share bounded content services; PWA shares strict validators and both HTML/JSON sinks; Global Webhooks shares secret-safe DNS-aware operations; Visibility shares every setting group and GIF branding storage; Translation shares literal bounded search plus exact-ID create/edit/confirmed-delete operations. All nine People panes are complete: Roles shares its invite-role checklist and capability matrix; Shared templates shares its three filters and grouping; Login shares all five allow settings, authentication identity fields and invitations; Email shares transport selection, write-only credentials, test delivery, invite-domain and address-change policy; Domains shares bounded search, counts and paging; Organizations shares its ten columns, creation/editing, tenant fields and GIF branding, three single/bulk feature flags, same-organization restriction, member-admin management and guarded deletion; Teams shares its ten columns, creation/editing, three single/bulk feature flags, same-Team restriction and guarded deletion; Locked Users shares all six protection settings, current lock details and individual/all unlock operations through fixed common services; People shares tenant-scoped discovery, five filters, nine list columns, locations, complete account creation/editing, active and lock state, GIF avatar history, Team bulk actions, confirmed deletion and audited impersonation. All People URLs expose signed inter-pane navigation rather than the generic route fallback. Anonymous and non-admin requests receive no admin data, and forged writes or refused sensitive reads are Security-reported. All ten Attachments panes are complete and share guarded settings, normalized limits, persisted move state, repair, server paths, storage counts, compaction, secret-safe bounded cloud configuration, migration progress/actions and scoped backup/schedule/list/restore operations with Jade |
| Account preferences and information | The Member Menu is complete at stable same URLs in Jade and HTML4. `/account/profile`, `/account/language`, `/account/password`, `/account/settings`, `/account/color`, `/account/font` and `/account/avatar` share account-bound boundaries for identity, language metadata, bounded password verification, typed preferences, catalogued appearance and owner-scoped GIF avatars. `/account/invite` shares normalized bounded email handling and lists only active-admin, non-archived boards; the service repeats that scope so an injected board ID is refused and Security-reported. `/account/logout` is an explicit submit in both views; HTML4 removes the exact rotated cookieless session and returns POST/303 to Sign In without issuing another authenticated token. Same-URL tests prove persisted positive behavior, paired views, forged profile/tag/secret/setting/theme/font/avatar/invitation refusals and terminal logout. |
| Unknown route (`*`) | Complete localized semantic Not Found table with HTTP 404. The capability probe remains HTTP 200 so a capable browser upgrades to Meteor's Not Found view. A live same-URL Chromium test checks both response modes and captures paired screenshots |
