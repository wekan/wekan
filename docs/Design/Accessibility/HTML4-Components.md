...
| Admin Panel pages and panes | `/admin/problems/summary` has a dedicated admin-only controller sharing the modern overview, new-problem query, stream allowlist, acknowledgement and broken-card/list-swimlane repair services. The Security, Speed, Tests, CPU, Database, Filesystem Integrity and API panes use one admin-only EventLog controller with their modern columns, bounded search, 10-row paging, legacy-address classification and stored username/location detail; CPU includes current load and API retains count-first ordering. Offices shares the modern person-first service, old-address fallback, 500-character search and 25-person pages, with every address, per-person count, IPv4/IPv6, trusted flag/city and time window intact. Every Problems pane is signed navigation. Anonymous and non-admin requests receive no report data; refused mutations are security-reported. Same-URL no-JavaScript/modern browser tests cover localization, acknowledgement, data isolation, report search, paging and IPv4/IPv6/location parity. The other Admin Panel panes still use the generic baseline and need dedicated controllers |
