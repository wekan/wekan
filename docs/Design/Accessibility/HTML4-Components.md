...
| `/import`, `/import/:source` | Shared source picker and import-part toggles; bounded signed JSON/CSV text and streamed JSON, Excel, Trello ZIP and WeKan ZIP files call their common sanitized importers. Whole-board JSON/CSV member mapping uses a short-lived session-bound sanitized draft, source-ordered labelled native username fields, exact-username suggestions, validated target accounts and an atomic single-use finish; Excel and multi-board ZIP retain Jade's direct no-mapping path, and WeKan ZIP attachments stream into Default Storage. Trello JSON and multi-board ZIP imports preserve the same optional named personal-workspace destination as Jade through a bounded server-only draft and authenticated find/create/assign operation. Trello API import remains |
