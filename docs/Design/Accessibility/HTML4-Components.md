...
| Admin Panel pages and panes | `/admin/problems/summary` has a dedicated admin-only controller sharing the modern overview, new-problem query, stream allowlist, acknowledgement and broken-card/list-swimlane repair services. Performance has a dedicated read-only controller sharing the translated Card loading guidance. Security exposes all eight translated settings and Notifications all three through signed labelled select forms and one pane-allowlisted, reported server operation shared with Jade. Delete shares its permanent-delete switch, Recovery guidance and fully audited server service with Jade. The Security Report, Speed, Tests, CPU, Database, Filesystem Integrity and API panes use one admin-only EventLog controller with their modern columns, bounded search, 10-row paging, legacy-address classification and stored username/location detail; CPU includes current load and API retains count-first ordering. Offices shares the modern person-first service, old-address fallback, 500-character search and 25-person pages, with every address, per-person count, IPv4/IPv6, trusted flag/city and time window intact. Impersonation Report shares one bounded admin service with its Meteor publication, retaining newest-first date, administrator, impersonated user, board and reason columns, ten-row search/paging, deleted-user id fallback and long-name wrapping. Recovery shares its admin service, All/Done/Failed/Deleted filter and bounded search/paging; all nine modern fields and textual outcome states remain available. Boards Report shares its instance-wide admin service and All/Public/Private filter; all seven columns resolve active member, Organization and Team names through page-local bounded lookups. Cards Report shares its bounded instance-wide service and resolves all six columns through four deduplicated page-local context queries. Broken Cards shares its four-branch definition and seven-column bounded admin service, with explicit Unknown context. Files Report shares one plain-collection metadata service, all seven columns, purpose-bound GIF/original controls and setting-gated audited permanent delete. Rules Report shares one bounded instance-wide service, its four modern columns and three deduplicated page-local Board/Action/Trigger lookups. Every Problems pane is signed navigation. Anonymous and non-admin requests receive no report data; refused mutations are security- or Recovery-reported as appropriate. Same-URL no-JavaScript/modern browser tests cover localization, acknowledgement, Performance guidance, Security, Delete and Notifications setting round trips, data isolation, report search, filters, paging, attachment controls/deletion, rule context, long-name wrapping, healthy/broken board-card relationships and IPv4/IPv6/location parity. The other Admin Panel panes still use the generic baseline and need dedicated controllers |
