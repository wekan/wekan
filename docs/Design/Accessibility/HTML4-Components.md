...
| Admin Panel pages and panes | `/admin/problems/summary` has a dedicated admin-only controller sharing the modern overview, new-problem query, stream allowlist, acknowledgement and broken-card/list-swimlane repair services. It renders every Problems pane as signed navigation, in-progress work and detected problems as text, and repeatable labelled checkboxes in one fieldset. Anonymous and non-admin requests receive no report data; refused mutations are security-reported. A same-URL no-JavaScript/modern browser test covers navigation, localization, acknowledgement and data isolation. The other Admin Panel panes still use the generic baseline and need dedicated controllers |
