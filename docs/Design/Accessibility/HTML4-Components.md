...
| Board rules | First dedicated slice: a board-visible list and localized trigger/action details use the same scoped documents as Jade. Active board/site admins receive labelled rename and two-step delete POST controls backed by the same exact-board service as HTML5; list, details, mutation, denial and same-URL screenshot tests are required. The trigger/action creation wizard, workflow graph and import/export remain |
