...
| `/allboards/...`, `/templates`, `/remaining`, `/archive`, `/public` | Dedicated readers with Starred, Remaining, Templates, Home, Archive and nested Workspace filtering. Signed per-board Star, Home, Restore and confirmed Archive controls call the same exact-scope operations as Meteor and report forged boards. Creation, duplication, permanent delete and Workspace assignment remain |
