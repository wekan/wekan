...
* **[minicardLabelText.js](../../client/lib/minicardLabelText.js)**: reads and writes the personal `Hide minicard label text` setting (the user's profile, or this browser when nobody is logged in), used by the minicard and by `Board Settings / Card`'s "Labels text" row;
