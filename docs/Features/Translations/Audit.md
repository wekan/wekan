...
| Pending review or repair | 1,748 |
...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **15,606** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `2acd6cc83` — replaced 25 Italian-seeded Venetian removal, restoration, checklist and insertion messages; preserved every placeholder and removed a stray digit before the subtask placeholder. Direct wording remains low confidence pending fluent-speaker review. Venetian has **366** original findings remaining. Registry verification confirms `ve-CC` is Venetian and `ve-PP` is Veps. Main Venda original findings are repaired in `180ca33c5`; restored and unflagged values still require validation. Wider repairs continue.
