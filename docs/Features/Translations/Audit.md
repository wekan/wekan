...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **16,073** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `29ee41f45` — replaced nine Finnish Veps password/settings/save/delete/cancel/help labels with software-attested native vocabulary from the directly fetched MediaWiki Veps translation file. All **16,073** correction records and vocabulary/token checks pass. These unflagged repairs leave original pending counts unchanged. The Veps year warning's digit terminology/grammar and composed calendar names need fluent review. Broader wrong-language/script values, restored translations and browser verification remain outstanding.
