...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **16,063** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `8b01dc136` — restored three canonical Veps date-format tokens and replaced a Finnish invalid-time warning with native vocabulary. The composed warning needs fluent review. All **16,063** correction records and vocabulary/token checks pass. These four unflagged repairs leave original pending counts unchanged. Broader wrong-language/script values, restored translations and browser verification remain outstanding.
