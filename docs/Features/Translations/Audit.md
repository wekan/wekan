...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **16,064** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `247ae2a9a` — replaced a Zulu Veps year warning with a direct Veps composition, preserving the all-four-digit instruction and 2026 example. **Low confidence:** digit terminology and sentence grammar need fluent review; dictionary evidence covers the remaining vocabulary. All **16,064** correction records and vocabulary/token checks pass. This unflagged repair leaves original pending counts unchanged. Broader wrong-language/script values, restored translations and browser verification remain outstanding.
