...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **16,034** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `56c02baca` — replaced Arabic Title/Language labels in Standard Moroccan Tamazight (`zgh`) with IRCAM-attested Tifinagh. All **16,034** correction records and vocabulary/script/token checks pass. These were unflagged findings; 598 original Tamazight findings remain. The broader file still mixes French, Arabic and Latin-script values requiring review. Veps/Acehnese wrong-language values, Uzbek Arabic script repairs, uncertain color borrowings and browser verification remain outstanding.
