...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **16,096** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `ab9562bad` — repaired eight Finnish/Zulu Veps sign-in/sign-out/email/yes/OK/confirmation labels with directly verified native software terms. Existing correct No remains Ei. All **16,096** correction records and vocabulary/token checks pass. These unflagged repairs leave original pending counts unchanged. Year-warning digit terminology, lockout/calendar compositions, broader wrong-language/script values, restored translations and browser verification remain outstanding.
