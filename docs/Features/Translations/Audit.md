...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **17,625** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-14**, local commit `b63e5ebe1` — replaced French Tamazight card-move rule trigger/fragments, including one unflagged fragment. Preserve when-event and another-list destination. Native movement components are supported; derived passive, agreement and full trigger remain **low confidence** pending language review. All **17,625** correction/rendering checks pass. **529** originals remain pending across **17** locales, including **391** Tamazight. Restored/unflagged values and earlier low-confidence wording still need validation. Live browser verification was not run; no translations were pushed.
