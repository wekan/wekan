...
**Upload workflow repair (2026-09-13):** local commit `27a566a0d` maps Colombian Spanish `es-CO` to supported Transifex `es_CO`, reports the legacy `es_CO` alias and uploads each distinct target once. Offline regressions pass; no upload was run. Nine differences between the two local Colombian files remain to reconcile.
