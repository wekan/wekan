...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **16,166** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `88220b7a0` — replaced five Finnish/Venda Veps account/email messages, resolving three original findings. Site placeholders, personal password reset, address verification and SMTP success are preserved. All **16,166** correction records and vocabulary/token checks pass. **Low confidence:** composed subjects and SMTP success grammar need fluent review. Invitation-subject terminology, earlier compositions, broader wrong-language/script values, restored translations and browser verification remain outstanding.
