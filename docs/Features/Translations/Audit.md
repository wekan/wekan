...
| Pending review or repair | 1,773 |
...
Fixed: wrong-language text, terminology, warnings, placeholders and JSON/calendar/search formatting. [Correction records](../../../releases/translations/audited-corrections.json) contain **15,581** exact before/after values, including unflagged repairs.

Latest translation fix: **2026-09-13**, local commit `65f266e36` — replaced 25 Italian-seeded Venetian activity messages, preserving event actions, source/destination hierarchy and every placeholder. Direct kanban wording is low confidence and awaits fluent-speaker review. Venetian has **391** original findings remaining. Registry verification confirms `ve-CC` is Venetian and `ve-PP` is Veps, not regional Venda. Main Venda original findings are repaired in `180ca33c5`; restored and unflagged values still require validation. Wider repairs continue.
