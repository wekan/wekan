...
[[ -n "$ISO_PATH" ]] || fail "usage: $0 /path/to/ARM64-install.iso"
...
echo "Installing Ubuntu virtualization packages on the host."
sudo apt-get update
...
# System libvirt cannot normally traverse a user's home directory, while
# AppArmor permits its managed image directory. Stage the verified ISO there
# instead of weakening the host boundary.
sudo install -o root -g root -m 0644 "$ISO_PATH" "$INSTALL_ISO_PATH"
...
The graphical installer will open in virt-manager. Complete guest installation
