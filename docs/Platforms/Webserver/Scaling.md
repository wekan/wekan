...
- Do not store attachments at database, like uploading file to card. Have markdown links to files, like `[Document](https://example.com/files/document.doc)`. Click `☰` at an opened card => `Show on Card`. There uncheck `[_] Attachments` to hide attachments at card. The same table, for what a MINICARD shows, is at `☰` on a card on the board => `Show on Minicard`.
...
Minimum specs:
- [RasPi3](../FOSS/HW/RaspberryPi/Raspberry-Pi.md), 1 GB RAM, external SSD disk for Wekan and MongoDB.
...
[AWS](../Propietary/Cloud/AWS)

[Azure OIDC](../../Features/Login/Azure/Azure.md)